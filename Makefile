...
ROM := pokeemerald-edit_modern.gba
...
.PHONY: all rom clean tidy tools mostlyclean clean-tools $(TOOLDIRS) berry_fix libagbsyscall modern debug modern_debug date git_hash

infoshell = $(foreach line, $(shell $1 | sed "s/ /__SPACE__/g"), $(info $(subst __SPACE__, ,$(line))))

# Build tools when building the rom
# Disable dependency scanning for clean/tidy/tools
ifeq (,$(filter-out all rom modern berry_fix libagbsyscall,$(MAKECMDGOALS)))
...
$(C_BUILDDIR)/%.o: $(C_SUBDIR)/%.s $$(c_asm_dep)
	@echo $<
	@$(CPP) -DMODERN=$(MODERN) -iquote include $< | $(AS) $(ASFLAGS) -o $@
