...
#define B_BADGE_BOOST               GEN_5 // In Gen4+, Gym Badges no longer boost a Pokémon's stats.
...
#define B_SPORT_DMG_REDUCTION       GEN_5 // In Gen5+, Water/Mud Sport reduce Fire/Electric Damage by 67% instead of 50%.
...
#define B_TAILWIND_TURNS            GEN_5 // In Gen5+, Tailwind lasts 4 turns instead of 3.
...
#define B_RECALC_TURN_AFTER_ACTIONS GEN_5 // In Gen8, switching/using a move affects the current turn's order of actions.

// Move data settings
#define B_UPDATED_MOVE_DATA         GEN_6 // Updates move data in gBattleMoves, including Power, Accuracy, PP, stat changes, targets, chances of secondary effects, etc.
#define B_PHYSICAL_SPECIAL_SPLIT    GEN_5 // In Gen3, the move's type determines if it will do physical or special damage. The split icon in the summary will reflect this.
...
#define B_RECOIL_IF_MISS_DMG        GEN_5 // In Gen5+, Jump Kick and High Jump Kick will always do half of the user's max HP when missing.
...
#define B_BLIZZARD_HAIL             GEN_5 // In Gen4+, Blizzard bypasses accuracy checks if it's hailing.
...
#define B_CAN_SPITE_FAIL            GEN_5 // In Gen4+, Spite can no longer fail if the foe's last move only has 1 remaining PP.
#define B_CRASH_IF_TARGET_IMMUNE    GEN_5 // In Gen4+, The user of Jump Kick or High Jump Kick will "keep going and crash" if it attacks a target that is immune to the move.
#define B_MEMENTO_FAIL              GEN_5 // In Gen4+, Memento fails if there is no target or if the target is protected or behind substitute. But not if Atk/Sp. Atk are at -6.
#define B_GLARE_GHOST               GEN_5 // In Gen4+, Glare can hit Ghost-type Pokémon normally.
#define B_SKILL_SWAP                GEN_5 // In Gen4+, Skill Swap triggers switch-in abilities after use.
...
#define B_WISH_HP_SOURCE            GEN_5 // In Gen5+, Wish heals half of the user's max HP instead of the target's.
#define B_RAMPAGE_CANCELLING        GEN_5 // In Gen5+, a failed Thrash, etc, will cancel except on its last turn.
...
#define B_ROOTED_GROUNDING          GEN_5 // In Gen4+, Ingrain causes the affected Pokémon to become grounded.
#define B_GROWTH_UNDER_SUN          GEN_5 // In Gen5+, Growth's effects are doubled when under the effects of the sun.
...
#define B_SHADOW_TAG_ESCAPE         GEN_5 // In Gen4+, if both sides have a Pokémon with Shadow Tag, all battlers can escape. Before, neither side could escape this situation.
...
// Item settings
#define B_HP_BERRIES                GEN_5 // In Gen4+, berries which restore hp activate immediately after HP drops to half. In Gen3, the effect occurs at the end of the turn.
#define B_BERRIES_INSTANT           GEN_5 // In Gen4+, most berries activate on battle start/switch-in if applicable. In Gen3, they only activate either at the move end or turn end.
...
#define B_SOUL_DEW_BOOST            GEN_5 // In Gens3-6, Soul Dew boosts Lati@s' Sp. Atk and Sp. Def. In Gen7+ it boosts the power of their Psychic and Dragon type moves instead.
#define B_NET_BALL_MODIFIER         GEN_7 // In Gen7+, Net Ball's catch multiplier is x5 instead of x3.
#define B_DIVE_BALL_MODIFIER        GEN_5 // In Gen4+, Dive Ball's effectiveness increases by when Surfing or Fishing.
...
#define B_DUSK_BALL_MODIFIER        GEN_5 // In Gen7+, Dusk Ball's catch multiplier is x3 instead of x3.5.
#define B_QUICK_BALL_MODIFIER       GEN_5 // In Gen5+, Quick Ball's catch multiplier is x5 instead of x4.
#define B_LURE_BALL_MODIFIER        GEN_7 // In Gen7+, Lure Ball's catch multiplier is x5 instead of x3.
#define B_HEAVY_BALL_MODIFIER       GEN_5 // In Gen7+, Heavy Ball's ranges change. See Cmd_handleballthrow.
...
// #define B_LEEK_ALWAYS_CRIT          GEN_6 // In Gen6+, if a Farfetch'd or Sirfetch'd holding a Leek use a move with increased Critical Hit ratio, it will always result in a Critical Hit.
#define B_SERENE_GRACE_BOOST        GEN_5 // In Gen5+, Serene Grace boosts the added flinch chance of King's Rock and Razor Fang.
...
#define B_SECRET_POWER_EFFECT       GEN_5 // Secret Power's effects change depending on terrain and generation. See GetSecretPowerMoveEffect.
#define B_SECRET_POWER_ANIMATION    GEN_5 // Secret Power's animations change depending on terrain and generation.
#define B_NATURE_POWER_MOVES        GEN_5 // Nature Power calls different moves depending on terrain and generation. See sNaturePowerMoves.
#define B_CAMOUFLAGE_TYPES          GEN_5 // Camouflage changes the user to different types depending on terrain and generation. See sTerrainToType.
...
// Catching settings
#define B_SEMI_INVULNERABLE_CATCH   GEN_5 // In Gen4+, you cannot throw a ball against a Pokemon that is in a semi-invulnerable state (dig/fly/etc)
#define B_CATCHING_CHARM_BOOST      0     // % boost in Critical Capture odds if player has the Catching Charm.
...
#define B_MULTI_BATTLE_WHITEOUT     GEN_5 // In Gen4+, multi battles end when the Player and also their Partner don't have any more Pokémon to fight.
