...
#define B_MULTI_HIT_CHANCE          GEN_5 // In Gen5+, multi-hit moves have different %. See Cmd_setmultihitcounter for values.
...
#define B_PSYWAVE_DMG               GEN_5 // Psywave's damage formula. See Cmd_psywavedamageeffect.
#define B_BADGE_BOOST               GEN_5 // In Gen4+, Gym Badges no longer boost a Pokémon's stats

// Move data settings
#define B_UPDATED_MOVE_DATA         GEN_5 // Updates move data in gBattleMoves, including Power, Accuracy, PP, stat changes, targets, chances of secondary effects, etc.
...
#define B_KINGS_SHIELD_LOWER_ATK    GEN_5 // In Gen7+, it lowers Atk by 1 stage instead of 2 of oponents that hit it.
...
#define B_TOXIC_NEVER_MISS          GEN_5 // In Gen6+, if Toxic is used by a Poison type, it will never miss.
...
#define B_CRASH_IF_TARGET_IMMUNE    GEN_4 // In Gen4+, The user of Jump Kick or Hi Jump Kick will "keep going and crash" if it attacks a target that is immune to the move.
...
#define B_GALE_WINGS                GEN_5 // In Gen7+ requires full HP to trigger.
...
#define B_MOODY_ACC_EVASION         GEN_5 // In Gen8+, Moody CANNOT raise Accuray and Evasion any more.
#define B_FLASH_FIRE_FROZEN         GEN_5 // In Gen5+, Flash Fire can trigger even when frozen, when it couldn't before.
#define B_SYNCHRONIZE_NATURE        GEN_5 // In Gen8+, if the Pokémon with Synchronize is leading the party, it's 100% guaranteed that wild Pokémon will have the same ability, as opposed to 50% previously.
...
#define B_STEEL_RESISTANCES         GEN_5 // In Gen6+, Steel-type Pokémon are no longer resistant to Dark and Ghost moves.
...
#define HIDE_HEALTHBOXES_DURING_ANIMS   FALSE    //if TRUE, hides healthboxes during move animations
